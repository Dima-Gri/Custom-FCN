...
class Linear(Module):
    ...
    def compute_output(self, input: np.ndarray) -> np.ndarray:
        ...
        """
        return input @ self.weight.T if self.bias is None else input @ self.weight.T + self.bias
...
class BatchNormalization(Module):
    ...
    def compute_output(self, input: np.ndarray) -> np.ndarray:
        ...
        if self.training:
            ...
            self.running_var = (1 - self.momentum) * self.running_var + self.momentum * (input.shape[0] / (input.shape[0] - 1)) * self.var
        else:
